...
from database import Notification, get_db, Item, ClaimRequest, ActivityLog
...
WHATSAPP_NUMBER = os.getenv("OFFICIAL_WA_NUMBER", "6289530428832")
SCHOOL_NAME = os.getenv("SCHOOL_NAME", "SMK Negeri 1 Surabaya")
ADMIN_NAME = "Zaki"
...
async def item_detail(request: Request, code: str, db: Session = Depends(get_db)):
    ...
    wa_message = (
        f"Halo, saya ingin mengklaim barang dengan kode {item.code} - {item.title}"
    )
...
async def submit_claim(
    ...
    if item.status != "FOUND":
        raise HTTPException(
            status_code=400,
            detail="Hanya barang berstatus Ditemukan yang bisa diklaim.",
        )
    ...
    db.add(
        ActivityLog(
            activity_type="NEW_CLAIM",
            description=f"{claimer_name} mengajukan klaim untuk barang '{item.title}'",
        )
    )
    db.add(
        Notification(
            message=f"Klaim baru dari {claimer_name} untuk barang {item.code}.",
            link="/admin/claims",
        )
    )
...
async def report_page(request: Request):
    return templates.TemplateResponse(
        "public_report.html", {"request": request, "school_name": SCHOOL_NAME}
    )
...
async def submit_report(
    ...
    db.add(
        Notification(
            message=f"Laporan baru dari {reporter_name} untuk barang '{title}'.",
            link="/admin/reviews",
        )
    )
